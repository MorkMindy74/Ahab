...
class SingleStateGRPOEnv:
    ...
    """
    def __init__(self, group_size, assets_filepath, start_date, end_date, initial_cash, gamma=0.99):
    ...
    def _discount_advantages(self, episode_advantage, episode_length):
        """
        Compute per-step discounted advantages from the episode-level advantage.
        Later steps (closer to terminal outcome) receive stronger signal via
        gamma-discounting from the end of the episode.
        """
    ...
    def collect_group_data_into_buffer(self, agent):
        ...
        2. Calculates group-relative advantages with per-step discounting.
        3. Fills the agent's buffer.
